...
glibc_version := 2.17
plugin_name := huawei_hilink-telegraf-plugin
...
.PHONY: $(plugin_name)
$(plugin_name):
	go build -ldflags "$(LDFLAGS)" ./cmd/huawei_hilink-telegraf-plugin

# Used by dockerfile builds
.PHONY: go-install
go-install:
	go install -mod=mod -ldflags "-w -s $(LDFLAGS)" ./cmd/huawei_hilink-telegraf-plugin
...
$(buildbin):
	@mkdir -pv $(dir $@)
	go build -o $(dir $@) -ldflags "$(LDFLAGS)" ./cmd/huawei_hilink-telegraf-plugin
